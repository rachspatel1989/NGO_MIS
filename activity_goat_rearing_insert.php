...
// receiving the post params
    ...
    if ($cnt == 0) {
        ...
        $reg_via = "Web";
        $bfcont = $_POST['beneficiary_amount'];
        $dbmgf = $_POST['amount_grant'];
        $farmercont = $_POST['farmers_contribution'];
        $mf = $_POST['mf'];
        $nabard = $_POST['nabard'];
        $othergrant = $_POST['othersB6'];
        $goats = $_POST['nos_goat'];
        $variety = $_POST['variety_goat'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query1 = mysql_query("INSERT INTO activity_progress_details_master(login_id,beneficiary_id,beneficiary_contribution,dbmgf_grant,farmers_contribution,mf_grant,nabard,other_grants,total_no,variety,reg_via,reg_device_id,reg_created_date,reg_timestamp) VALUES('$login_id','$benificiary_id','$bfcont','$dbmgf','$farmercont','$mf','$nabard','$othergrant','$goats','$variety','$reg_via','$ip',STR_TO_DATE('$date','%m/%d/%Y%h:%i'),'$timestamp')");
        ...
        $reg_via = "Web";
        $insurance_amt = $_POST['amount_insurance'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query2 = mysql_query("Update activity_progress_details_master set insurance_amt= '$insurance_amt', reg_modified_date= STR_TO_DATE('$date', '%m/%d/%Y%h:%i'), reg_timestamp= '$timestamp' WHERE beneficiary_id = '$benificiary_id'");
        ...
        $reg_via = "Web";
        $goats_survived = $_POST['nos_goats'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query3 = mysql_query("Update activity_progress_details_master set survival_no= '$goats_survived', reg_modified_date= STR_TO_DATE('$date', '%m/%d/%Y%h:%i'), reg_timestamp= '$timestamp' WHERE beneficiary_id = '$benificiary_id'");
