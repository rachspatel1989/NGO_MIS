...
// receiving the post params
    ...
    if ($cnt == 0) {
        ...
        $reg_via = "Web";
        $contribution_amt = $_POST['amount_contribution'];
        $activity_name = $_POST['activity_name'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query1 = mysql_query("INSERT INTO activity_progress_details_master(login_id,beneficiary_id,contribution_amt,activity_name,reg_via,reg_device_id,reg_created_date,reg_timestamp) VALUES('$login_id','$benificiary_id','$contribution_amt','$activity_name','$reg_via','$ip',STR_TO_DATE('$date','%m/%d/%Y%h:%i'),'$timestamp')");
        ...
        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query2 = mysql_query("Update activity_progress_details_master set enterprise_verification= '$enterprise', reg_modified_date= STR_TO_DATE('$date', '%m/%d/%Y%h:%i'), reg_timestamp= '$timestamp' WHERE beneficiary_id = '$benificiary_id'");
