...
// receiving the post params
    ...
    if ($cnt == 0) {
        ...
        $landarea = $_POST['levelled_landarea'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query1 = mysql_query("INSERT INTO activity_progress_details_master(login_id,beneficiary_id,dbmgf_grant,lhwrf_grant,mf_grant,beneficiary_contribution,nabard,other_grants,land_levelled_area,reg_via,reg_device_id,reg_created_date,reg_timestamp) VALUES('$login_id','$benificiary_id','$dbmgf','$lhwrf','$mf','$bfcont','$nabard','$othergrant','$landarea','$reg_via','$ip',STR_TO_DATE('$date','%m/%d/%Y%h:%i'),'$timestamp')");
