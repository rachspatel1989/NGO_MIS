...
// receiving the post params
    ...
    if ($cnt == 0) {
        ...
        $reg_via = "Web";
        $no_of_plants = $_POST['no_of_plant'];
        $species_name = $_POST['species_name'];
        $variety = $_POST['variety'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query1 = mysql_query("INSERT INTO activity_progress_details_master(login_id,beneficiary_id,total_no,species_name,variety,reg_via,reg_device_id,reg_created_date,reg_timestamp) VALUES('$login_id','$benificiary_id','$no_of_plants','$species_name','$variety','$reg_via','$ip',STR_TO_DATE('$date','%m/%d/%Y%h:%i'),'$timestamp')");
        ...
        $reg_via = "Web";
        $plant_survival = $_POST['survival_no'];
        ...
        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query2 = mysql_query("Update activity_progress_details_master set dbmgf_grant= '$dbmgf', lhwrf_grant= '$lhwrf', mf_grant= '$mf', beneficiary_contribution= '$bfcont', nabard= '$nabard', other_grants= '$othergrant',survival_no= '$plant_survival', reg_modified_date= STR_TO_DATE('$date','%m/%d/%Y%h:%i'), reg_timestamp= '$timestamp' WHERE beneficiary_id = '$benificiary_id'");
        ...
        $reg_via = "Web";
        $equip_name = $_POST['equipment_name'];
        $equip_nos = $_POST['equipment_nos'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query3 = mysql_query("Update activity_progress_details_master set equipment_name= '$equip_name', equipment_no= '$equip_nos', reg_modified_date= STR_TO_DATE('$date','%m/%d/%Y%h:%i'), reg_timestamp= '$timestamp' WHERE beneficiary_id = '$benificiary_id'");
