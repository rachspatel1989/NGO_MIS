...
// receiving the post params
    ...
    if ($cnt == 0) {
        ...
        $poultry_chicks = $_POST['poultry_chicks'];
        $variety = $_POST['poultry_chicks_variety'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query1 = mysql_query("INSERT INTO activity_progress_details_master(login_id,beneficiary_id,dbmgf_grant,farmers_contribution,mf_grant,nabard,other_grants,total_no,variety,reg_via,reg_device_id,reg_created_date,reg_timestamp) VALUES('$login_id','$benificiary_id','$dbmgf','$farmercont','$mf','$nabard','$othergrant','$poultry_chicks','$variety','$reg_via','$ip',STR_TO_DATE('$date','%m/%d/%Y%h:%i'),'$timestamp')");
        ...
        $reg_via = "Web";
        $chick_survival = $_POST['survived_chicks'];

        $result = $db->insertActivityTracker($login_id, $family_id, $allocate_id, $benificiary_id, $indicator, $action, $visit, $remark, $ip, $date, $timestamp);
        $query2 = mysql_query("Update activity_progress_details_master set survival_no= '$chick_survival', reg_modified_date= STR_TO_DATE('$date', '%m/%d/%Y%h:%i'), reg_timestamp= '$timestamp' WHERE beneficiary_id = '$benificiary_id'");
