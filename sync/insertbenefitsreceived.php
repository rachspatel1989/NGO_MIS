...
$json = $_POST["benefitsreceivedJSON"];
...
for ($i = 0; $i < count($data); $i++) {
    ...
    $recievebenefit = $data[$i]->recievebenefit;
    $schemereceive = $data[$i]->schemereceive;
    $rate = $data[$i]->rate;
    $ip = $data[$i]->ip;
    ...
    if (isset($ip) && isset($tempid)) {
        ...
        if ($family_id != "") {
            ...
            $result = $db->BenefitsReceived($login_id, $family_id, mysql_real_escape_string($recievebenefit), mysql_real_escape_string($schemereceive), mysql_real_escape_string($rate), $ip, $tempid, $date, $timestamp);

            //Based on insertion, create JSON response
            if ($result) {
                $benefitsreceived["reg_temp_id"] = $tempid;
                $benefitsreceived["status"] = 'yes';
                array_push($response, $benefitsreceived);
            } else {
                $benefitsreceived["reg_temp_id"] = $tempid;
                $benefitsreceived["status"] = 'no';
                array_push($response, $benefitsreceived);
            }
        }
    } else {
