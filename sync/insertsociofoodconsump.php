...
$json = $_POST["sociofoodconsumpJSON"];
...
for ($i = 0; $i < count($data); $i++) {
    ...
    $buyfood = $data[$i]->buyfood;
    $consumefood = $data[$i]->consumefood;
    $quantityoffood = $data[$i]->quantityoffood;
    $receivedwages = $data[$i]->receivedwages;
    $costfood = $data[$i]->costfood;
    ...
    if (isset($ip) && isset($tempid)) {
        ...
        if ($family_id != "") {
            ...
            $result = $db->Sociofoodconsumption($login_id, $family_id, mysql_real_escape_string($buyfood), mysql_real_escape_string($consumefood), mysql_real_escape_string($quantityoffood), mysql_real_escape_string($receivedwages), mysql_real_escape_string($costfood), mysql_real_escape_string($rainfall), $ip, $date, $timestamp, $tempid);
