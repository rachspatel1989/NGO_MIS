...
$json = $_POST["sociononfoodexpenseJSON"];
...
for ($i = 0; $i < count($data); $i++) {
    ...
    $expense_id = $data[$i]->expense_id;
    $cost_per_year = $data[$i]->cost_per_year;
    $ip = $data[$i]->ip;
    ...
    if (isset($ip) && isset($tempid)) {
        ...
        if ($family_id != "") {
            ...
            $result = $db->Sociononfood($login_id,$family_id,mysql_real_escape_string($expense_id),mysql_real_escape_string($cost_per_year),$ip,$tempid,$date,$timestamp);
