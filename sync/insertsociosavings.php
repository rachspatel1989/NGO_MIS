...
for ($i = 0; $i < count($data); $i++) {
    ...
    $saving_source = $data[$i]->saving_source;
    $household_savings = $data[$i]->household_savings;
    $saved_rupees = $data[$i]->saved_rupees;
    $ip = $data[$i]->ip;
    ...
    if (isset($ip) && isset($tempid)) {
        ...
        if ($family_id != "") {
            ...
            $result = $db->Sociosavings($login_id,$family_id,mysql_real_escape_string($saving_source),mysql_real_escape_string($household_savings),mysql_real_escape_string($saved_rupees),$ip,$tempid,$date,$timestamp);
